...
class StudentRollController extends Controller
{
    ...
    public function GetStudent(Request $request){
    	// dd('ok done');
    	$allData = AssignStudent::with(['student'])->where('year_id',$request->year_id)->where('class_id',$request->class_id)->where("stream_id",$request->stream_id)->get();
    	// dd($allData->toArray());
    ...
    public function StudentRollStore(Request $request){
    	...
    	if ($request->student_id !=null) {
    		for ($i=0; $i < count($request->student_id); $i++) {
    			AssignStudent::where('year_id',$year_id)->where('class_id',$class_id)->where('stream_id',$stream_id)->where('student_id',$request->student_id[$i])->update(['roll' => $request->roll[$i]]);
    		} // end for loop
    	}else{
    		$notification = array(
